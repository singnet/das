...
async fn start_grpc_server() -> Result<(), Box<dyn std::error::Error>> {
    println!("Inside gRPC server thread");
    let addr = "[::1]:9090".parse()?;
    let atom_space_node = MyAtomSpaceNode::default();

    Server::builder()
        .add_service(AtomSpaceNodeServer::new(atom_space_node))
        .serve(addr)
        .await?;
...
fn start_grpc_server_in_background() {
    println!("Spawning DAS gRPC Server on port 9090.");
    thread:: spawn(|| {
