...
from hyperon_das.grpc import common_pb2
from hyperon_das.grpc import attention_broker_pb2
...
class AttentionBrokerGateway:
    ...
    def stimulate(self, handle_count: Set[str], context: str = "") -> Optional[str]:
        ...
        message = attention_broker_pb2.HandleCount(map=handle_count, context=context)
        with grpc.insecure_channel(self.server_url) as channel:
            stub = AttentionBrokerStub(channel)
    ...
    def correlate(self, handle_set: Set[str], context: str = "") -> Optional[str]:
        ...
        message = attention_broker_pb2.HandleList(list=handle_set, context=context)
        sleep(0.05)
        with grpc.insecure_channel(self.server_url) as channel:
            stub = AttentionBrokerStub(channel)
