...
using CustomAttributes =
    unordered_map<CustomAttributesKey, CustomAttributesValue>;
 ...
 */
template <typename T>
static const optional<T>
get_custom_attribute(const CustomAttributes &custom_attributes,
 ...
 */
static string
custom_attributes_to_string(const CustomAttributes &custom_attributes) {
...
public:
   ...
   * to an empty map.
   */
  Atom(const string &id, const string &handle,
       const string &composite_type_hash, const string &named_type,
       const CustomAttributes &custom_attributes = CustomAttributes{})
      : _id(id), handle(handle), composite_type_hash(composite_type_hash),
        named_type(named_type), custom_attributes(custom_attributes) {}
...
public:
   ...
   * to an empty map.
   */
  Node(const string &id, const string &handle,
       const string &composite_type_hash, const string &named_type,
       const string &name,
       const CustomAttributes &custom_attributes = CustomAttributes{})
      : name(name),
...
public:
   ...
   * ```
   */
  Link(const string &type, const TargetsDocuments &targets,
       const CustomAttributes &custom_attributes = CustomAttributes{})
   ...
   * vector.
   */
  Link(const string &id, const string &handle,
       const string &composite_type_hash, const string &named_type,
       const ListOfAny &composite_type, const string &named_type_hash,
       const vector<string> &targets, bool is_toplevel,
       const CustomAttributes &custom_attributes = CustomAttributes{},
       const TargetsDocuments &targets_documents = TargetsDocuments{})
      : composite_type(composite_type), named_type_hash(named_type_hash),
        targets(targets), is_toplevel(is_toplevel),
  ...
  void validate() const override {
    ...
    if (not this->validate_composite_type(this->composite_type)) {
      throw invalid_argument("Invalid composite type. All elements must be "
  ...
  const string to_string() const noexcept override {
    ...
    result += ", composite_type: " +
              composite_type_list_to_string(this->composite_type);
  ...
  const string composite_type_list_to_string(
      const ListOfAny &composite_type) const noexcept {
    string result = "[";
