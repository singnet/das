 ...
 */
StringMatrix
multiply_binary_matrix_by_string_matrix(const IntMatrix &binary_matrix,
...
StringList build_pattern_keys(const StringList &hash_list) {
  ...
  auto result_matrix =
      multiply_binary_matrix_by_string_matrix(binary_matrix, hash_list);
  ...
  for (const auto &matrix_item : result_matrix) {
    ...
    keys.push_back(
        move(ExpressionHasher::expression_hash(type_hash, elements)));
  }
