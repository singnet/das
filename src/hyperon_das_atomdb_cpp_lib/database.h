 ...
 *
 * This header file defines the AtomDB class, which serves as an abstract base
 * class for interacting with the AtomDB database. The AtomDB class provides a
 * comprehensive interface for querying, adding, and managing atoms, nodes, and
 * links within the database. Derived classes must implement the pure virtual
 ...
 *
 * The class is designed to be extended by specific database implementations,
 * allowing for flexible and customizable database interactions. It includes
 * static methods for building node and link handles, as well as methods for
 * checking the existence of nodes and links, retrieving atoms, and reformatting
 * documents.
 *
 * The AtomDB class also defines several pure virtual methods that must be
 * implemented by derived classes. These methods cover a wide range of database
 * operations, such as retrieving node handles, querying the database by field
 * or text, managing links, reindexing, and more.
 *
 * @note This class uses several custom types and utility classes, such as
 * StringList, ExpressionHasher, NodeParams, LinkParams, and KwArgs, which are
...
/**
 * @brief A Plain Old Data (POD) type representing various boolean flags for
 * configuration options.
 *
 * This structure contains several boolean flags that control different aspects
 * of the configuration, such as target formatting, document handling,
 * representation depth, and scope of operation.
 */
struct KwArgs {
 ...
 * @brief Abstract base class for the AtomDB database interface.
 *
 * The AtomDB class defines the interface for interacting with the AtomDB
 * database. It provides various pure virtual methods for querying, adding, and
 * managing atoms, nodes, and links within the database. Derived classes must
 * implement these methods to provide concrete functionality.
 *
...
public:
   ...
   */
  static const string build_node_handle(const string &node_type,
   ...
   */
  static const string build_link_handle(const string &link_type,
    ...
    return ExpressionHasher::composite_hash(
        target_handle.empty()
            ? StringList{{move(link_type_hash)}}
   ...
   * @return A boolean value indicating whether the node exists (true) or not
   * (false).
   */
   ...
   * @param target_handles A vector of strings representing the handles of the
   * link's targets.
   * @return A boolean value indicating whether the link exists (true) or not
   * (false).
   */
  bool link_exists(const string &link_type,
                   const StringList &target_handles) const;

  /**
   * @brief Retrieves an atom from the database using its handle and optional
   * params.
   * @param handle A string representing the handle of the atom to be retrieved.
   * @param kwargs An optional Kwargs object containing additional retrieval
   * options, as follows: `no_target_format` (`bool`, optional): If True, return
   * the document without transforming it to the target format. Defaults to
   * False. `targets_document` (`bool`, optional): If True, include the
   * `targets_document` in the response. Defaults to False.
   *               `deep_representation` (`bool`, optional): If True, include a
   * deep representation of the targets. Defaults to False.
   * @return A const shared pointer to an Atom object.
   */
  const shared_ptr<const Atom> get_atom(const string &handle,
   ...
   * @return The node handle.
   */
  virtual const string get_node_handle(const string &node_type,
  ...
  /**
   * @brief Get the handles of (a) node(s) of the specified type containing the
   * given substring.
   * @param node_type The node type.
   ...
   * @param index_id The ID of the index to use for retrieving atoms from the
   * database.
   * @param query A vector of ordered maps representing the query parameters.
   * @param cursor An integer representing the cursor position for pagination
   * (default is 0).
   * @param chunk_size An integer representing the number of atoms to retrieve
   * in one chunk (default is 500).
   * @return A pair containing an cursor and a list of atoms. The cursor is used
   * for pagination or further retrieval operations, and the list contains the
   * retrieved atoms.
   */
  virtual const pair<const int, const AtomList>
  get_atoms_by_index(const string &index_id,
                     const vector<map<string, string>> &query, int cursor = 0,
  ...
  virtual const StringList get_atoms_by_text_field(
      const string &text_value, const optional<string> &field = nullopt,
   ...
   * @return List of node handles.
   */
  virtual const StringList
  get_node_by_name_starting_with(const string &node_type,
   ...
   */
  virtual const StringList
  get_all_nodes_handles(const string &node_type) const = 0;
   ...
   */
  virtual const StringList
  get_all_nodes_names(const string &node_type) const = 0;
   ...
   */
  virtual const StringUnorderedSet
  get_all_links(const string &link_type) const = 0;
  ...
  get_link_handle(const string &link_type,
   ...
   */
  virtual const StringList
  get_link_targets(const string &link_handle) const = 0;
   ...
   * @param kwargs An const reference to a Kwargs object containing additional
   * retrieval options.
   * @return A list of strings representing the incoming link handles.
   */
  virtual const StringList
  get_incoming_links_handles(const string &atom_handle,
   ...
   */
  virtual const vector<shared_ptr<const Atom>>
  get_incoming_links_atoms(const string &atom_handle,
                           const KwArgs &kwargs = {}) const = 0;
   ...
   */
  virtual const StringUnorderedSet
  get_matched_links(const string &link_type, const StringList &target_handles,
                    const KwArgs &kwargs = {}) const = 0;
   ...
   */
  virtual const StringUnorderedSet
  get_matched_type_template(const StringList &_template,
                            const KwArgs &kwargs = {}) const = 0;

  /**
   * @brief Retrieves matched types based on the specified link type.
   * @param link_type A string representing the type of the links to retrieve.
   * @param kwargs A const reference to a Kwargs object containing additional
   * retrieval options.
   * @return A set of handles that matched.
   */
  virtual const StringUnorderedSet
  get_matched_type(const string &link_type,
                   const KwArgs &kwargs = {}) const = 0;

  /**
   * @brief Retrieves the type of the atom with the specified handle.
   * @param handle A string representing the handle of the atom.
   * @return An optional string containing the type of the atom if found,
   * otherwise nullopt.
   */
  virtual const optional<const string>
  get_atom_type(const string &handle) const = 0;

  /**
   * @brief Count the total number of atoms in the database.
   * @return A dictionary containing the count of node atoms, link atoms, and
   * total atoms.
   */
   ...
   * @param node_params A NodeParams object containing the parameters for the
   * node.
   ...
   * @param link_params A LinkParams object containing the parameters for the
   * link.
   * @param toplevel A boolean indicating whether the link is a top-level link
   * (default is true).
   * @return A const shared pointer to a Link object.
   */
  virtual const shared_ptr<const Link> add_link(const Link &link_params,
                                                bool toplevel = true) = 0;

  /**
   * @brief Reindexes the inverted pattern index according to the passed
   * templates.
   *
   * This function reindexes the inverted pattern index based on the specified
   * pattern templates. The pattern templates are specified by atom type in a
   * map, where each atom type maps to a pattern template.
   *
   * @param pattern_index_templates A map where the keys are atom types and the
   * values are pattern templates. Each pattern template is a vector of maps,
   * where each map specifies a pattern template with:
   *        - "named_type": A boolean indicating whether the named type should
   * be included.
   *        - "selected_positions": A vector of integers specifying the selected
   * positions.
   *
   * Pattern templates are applied to each link entered in the atom space to
   * determine which entries should be created in the inverted pattern index.
   * Entries in the inverted pattern index are like patterns where the link type
   * and each of its targets may be replaced by wildcards. For instance, given a
   * similarity link Similarity(handle1, handle2), it could be used to create
   * any of the following entries in the inverted pattern index:
   *
   * - *(handle1, handle2)
   *
   * - Similarity(*, handle2)
   ...
   * - Similarity(*, *)
   *
   * If we create all possibilities of index entries for all links, the pattern
   * index size will grow exponentially, so we limit the entries we want to
   * create by each type of link. This is what a pattern template for a given
   * link type is. For instance, if we apply this pattern template:
   *
   * @code
   ...
   */
  virtual void
  reindex(const unordered_map<string, vector<unordered_map<string, any>>>
   ...
   */
  virtual const string create_field_index(
      const string &atom_type, const StringList &fields,
   ...
   * @param documents A list of Atom objects, each representing a document to be
   ...
   * @param kwargs A const reference to a Kwargs object containing the
   * reformatting options.
   * @return A shared pointer with a copy of the reference object but with the
   * new format.
   */
  const shared_ptr<const Atom>
  _reformat_document(const shared_ptr<const Atom> &document,
                     const KwArgs &kwargs = {}) const;
...
protected:
   ...
   * @param link_params A LinkParams object containing the parameters for the
   * link.
   * @param is_toplevel A boolean indicating whether the link is a top-level
   * link.
   * @return A shared pointer to a Link object.
   */
  virtual shared_ptr<Link> _build_link(const Link &link_params,
                                       bool is_toplevel = true);

  /**
   * @brief Retrieves an atom from the database using its handle.
   * @param handle A string representing the handle of the atom to be retrieved.
   * @return A const shared pointer to the Atom object representing the
   * retrieved atom.
   */
  virtual const shared_ptr<const Atom>
  _get_atom(const string &handle) const = 0;
};
