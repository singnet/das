...
//------------------------------------------------------------------------------
const string InMemoryDB::get_node_handle(const string &node_type,
...
//------------------------------------------------------------------------------
const StringList InMemoryDB::get_node_by_name(const string &node_type,
  ...
  for (const auto &[key, node] : this->db.node) {
    if (node->name.find(substring) != string::npos and
...
InMemoryDB::get_atoms_by_index(const string &index_id,
                               const vector<map<string, string>> &query,
                               int cursor, int chunk_size) const {
  throw runtime_error("Not implemented");
}

//------------------------------------------------------------------------------
const StringList InMemoryDB::get_atoms_by_text_field(
    const string &text_value, const optional<string> &field,
...
//------------------------------------------------------------------------------
const StringList
InMemoryDB::get_node_by_name_starting_with(const string &node_type,
...
//------------------------------------------------------------------------------
const StringList
InMemoryDB::get_all_nodes_handles(const string &node_type) const {
  auto node_type_hash = ExpressionHasher::named_type_hash(node_type);
  StringList node_handles;
...
//------------------------------------------------------------------------------
const StringList
InMemoryDB::get_all_nodes_names(const string &node_type) const {
  auto node_type_hash = ExpressionHasher::named_type_hash(node_type);
...
InMemoryDB::get_all_links(const string &link_type) const {
...
//------------------------------------------------------------------------------
const string
InMemoryDB::get_link_handle(const string &link_type,
  ...
  target_handles_str += "]";
  throw AtomDoesNotExist("Nonexistent atom",
                         link_type + ":" + target_handles_str);
}
...
//------------------------------------------------------------------------------
const StringList
InMemoryDB::get_incoming_links_handles(const string &atom_handle,
...
InMemoryDB::get_incoming_links_atoms(const string &atom_handle,
                                     const KwArgs &kwargs) const {
...
//------------------------------------------------------------------------------
const StringUnorderedSet
InMemoryDB::get_matched_links(const string &link_type,
                              const StringList &target_handles,
                              const KwArgs &kwargs) const {
  if (link_type != WILDCARD and
      find(target_handles.begin(), target_handles.end(), WILDCARD) ==
          target_handles.end()) {
    try {
  ...
  auto link_type_hash = link_type == WILDCARD
                            ? WILDCARD
                            : ExpressionHasher::named_type_hash(link_type);
...
//------------------------------------------------------------------------------
const StringUnorderedSet
InMemoryDB::get_matched_type_template(const StringList &_template,
                                      const KwArgs &kwargs) const {
...
//------------------------------------------------------------------------------
const StringUnorderedSet
InMemoryDB::get_matched_type(const string &link_type,
                             const KwArgs &kwargs) const {
  auto it =
      this->db.templates.find(ExpressionHasher::named_type_hash(link_type));
  if (it != this->db.templates.end()) {
    if (kwargs.toplevel_only) {
      return this->_filter_non_toplevel(it->second);
    }
...
InMemoryDB::get_atom_type(const string &handle) const {
...
//------------------------------------------------------------------------------
const shared_ptr<const Link> InMemoryDB::add_link(const Link &link_params,
                                                  bool toplevel) {
...
void InMemoryDB::reindex(
    const unordered_map<string, vector<unordered_map<string, any>>>
...
//------------------------------------------------------------------------------
const string InMemoryDB::create_field_index(
    const string &atom_type, const StringList &fields, const string &named_type,
...
//------------------------------------------------------------------------------
const shared_ptr<const Link>
InMemoryDB::_get_and_delete_link(const string &link_handle) {
...
//------------------------------------------------------------------------------
const StringList
InMemoryDB::_build_named_type_hash_template(const StringList &_template) const {
  StringList hash_template;
  hash_template.reserve(_template.size());
  for (const auto &element : _template) {
    hash_template.push_back(
        move(this->_build_named_type_hash_template(element)));
  }
...
//------------------------------------------------------------------------------
const string
InMemoryDB::_build_named_type_hash_template(const string &_template) const {
  return ExpressionHasher::named_type_hash(_template);
}

//------------------------------------------------------------------------------
void InMemoryDB::_add_outgoing_set(const string &key,
...
InMemoryDB::_get_and_delete_outgoing_set(const string &handle) {
...
//------------------------------------------------------------------------------
void InMemoryDB::_add_incoming_set(const string &key,
                                   const StringList &targets_hash) {
...
//------------------------------------------------------------------------------
void InMemoryDB::_add_patterns(const string &named_type_hash, const string &key,
                               const StringList &targets_hash) {
...
//------------------------------------------------------------------------------
void InMemoryDB::_delete_patterns(const Link &link_document,
                                  const StringList &targets_hash) {
...
//------------------------------------------------------------------------------
const StringUnorderedSet
InMemoryDB::_filter_non_toplevel(const StringUnorderedSet &matches) const {
...
void InMemoryDB::_add_atom_index(const Atom &atom) {
  if (auto link = dynamic_cast<const Link *>(&atom)) {
    ...
    this->_add_templates(link->composite_type_hash, link->named_type_hash,
                         handle);
