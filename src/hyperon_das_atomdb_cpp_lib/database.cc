...
//------------------------------------------------------------------------------
bool AtomDB::node_exists(const string &node_type,
...
//------------------------------------------------------------------------------
bool AtomDB::link_exists(const string &link_type,
...
//------------------------------------------------------------------------------
const shared_ptr<const Atom> AtomDB::get_atom(const string &handle,
...
AtomDB::_reformat_document(const shared_ptr<const Atom> &document,
                           const KwArgs &kwargs) const {
  if (const auto &link = dynamic_pointer_cast<const Link>(document)) {
    ...
    if (targets_document or deep_representation) {
      ...
      for (const auto &target : link->targets) {
        const auto &atom = deep_representation ? this->get_atom(target, kwargs)
                                               : this->get_atom(target);
        if (not atom)
          continue;
...
//------------------------------------------------------------------------------
shared_ptr<Link> AtomDB::_build_link(const Link &link_params,
                                     bool is_toplevel) {
  ...
  string handle =
      ExpressionHasher::expression_hash(link_type_hash, target_handles);
  string composite_type_hash =
      ExpressionHasher::composite_hash(composite_type_elements);

  auto link =
      make_shared<Link>(handle,                        // id
                        handle,                        // handle
                        composite_type_hash,           // composite_type_hash
