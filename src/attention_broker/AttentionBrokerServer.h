...
public:
  ...
  static const unsigned int WORKER_THREADS_COUNT =
      10; /// Number of working threads.

  // Stimuli spreading parameters
  string global_context;
  constexpr const static double RENT_RATE = 0.50; /// double in [0..1] range.
  constexpr const static double SPREADING_RATE_LOWERBOUND =
      0.01; /// double in [0..1] range.
  constexpr const static double SPREADING_RATE_UPPERBOUND =
      0.10; /// double in [0..1] range.
   ...
   * otherwise.
   */
  Status ping(ServerContext *grpc_context, const dasproto::Empty *request,
