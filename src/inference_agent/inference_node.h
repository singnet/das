...
public:
  ...
  message_factory(std::string &command, std::vector<std::string> &args);

  const std::string CREATE_INFERENCE = "create_inference";
  const std::string INFERENCE_ANSWER = "inference_answer";
  const std::string DISTRIBUTED_INFERENCE_FINISHED =
      "distributed_inference_finished";
...
public:
  DistributedInferenceFinishedMessage(std::string command,
