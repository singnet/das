...
double compute_sim1(const vector<string> &tokens1,
...
double compute_sim2(const vector<string> &tokens1,
                    const vector<string> &tokens2) {
...
string highlight(const vector<string> &tokens1, const vector<string> &tokens2,
...
void build_link(const string &link_type_tag, const string str1,
                const string str2, double threshold, stack<string> &output,
                const set<string> &highlighted) {
  ...
  if (v1 >= threshold) {
    // output.push(std::to_string(v1) + ": " + highlight(tokens1, tokens2,
    // highlighted)); output.push(std::to_string(v2) + ": " + highlight(tokens2,
    // tokens1, highlighted));
...
string handle_to_atom(const char *handle) {
  ...
  shared_ptr<atomdb_api_types::AtomDocument> document =
      db->get_atom_document(handle);
  shared_ptr<atomdb_api_types::HandleList> targets =
      db->query_for_targets((char *)handle);
  string answer;
...
void run(const string &context, const string &link_type_tag,
         const set<string> highlighted) {
  ...
  vector<string> query_same_word = {
      and_operator,  "2",        link_template, expression, "3",      node,
      symbol,        contains,   variable,      sentence1,  variable, word1,
      link_template, expression, "3",           node,       symbol,   contains,
      variable,      sentence2,  variable,      word1};

  vector<string> query_same_size{
      or_operator, "1",       link_template, expression, "4",
      node,        symbol,    similarity,    variable,   sentence1,
      variable,    sentence2, variable,      tv1};
  ...
  while (!response->finished()) {
    if ((query_answer = response->pop()) == NULL) {
      ...
      sentence_document1 = db->get_atom_document(
          query_answer->assignment.get(sentence1.c_str()));
      sentence_document2 = db->get_atom_document(
          query_answer->assignment.get(sentence2.c_str()));
      sentence_symbol_document1 =
          db->get_atom_document(sentence_document1->get("targets", 1));
      sentence_symbol_document2 =
          db->get_atom_document(sentence_document2->get("targets", 1));
      string s1 = string(sentence_symbol_document1->get("name"));
      string s2 = string(sentence_symbol_document2->get("name"));
      if ((already_inserted_links.find(s1 + s2) ==
           already_inserted_links.end()) &&
          (already_inserted_links.find(s2 + s1) ==
           already_inserted_links.end())) {
...
int main(int argc, char *argv[]) {
  if (argc < 3) {
    cerr << "Usage: " << argv[0] << " <context> <link type tag> <word>*"
         << endl;
    exit(1);
  }
