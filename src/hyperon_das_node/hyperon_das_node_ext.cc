...
NB_MODULE(hyperon_das_node_ext, m) {
  ...
  nb::class_<Message, MessageTrampoline>(m, "Message")
      .def(nb::init<>())
      .def("act",
           &Message::act); // Bind the act method (can be overriden in Python)

  nb::class_<MessageFactory, MessageFactoryTrampoline>(m, "MessageFactory")
      .def("message_factory",
           &MessageFactory::message_factory); // Bind the message_factory method
  ...
  nb::class_<DistributedAlgorithmNode, MessageFactory,
             DistributedAlgorithmNodeTrampoline>(m, "DistributedAlgorithmNode")
      ...
      .def("broadcast", &DistributedAlgorithmNode::broadcast, "command"_a,
           "args"_a)
      .def("send", &DistributedAlgorithmNode::send, "command"_a, "args"_a,
           "recipient"_a)
      .def("node_joined_network",
           &DistributedAlgorithmNode::node_joined_network, "node_id"_a)
      .def("cast_leadership_vote",
           &DistributedAlgorithmNode::cast_leadership_vote)
