...
ProtectedAtomDB::ProtectedAtomDB(shared_ptr<AtomDB> backend) : backend(backend) {
    ...
    if (this->backend->get_protection_mode() != atomdb_api_types::ProtectionMode::PROTECTED) {
        RAISE_ERROR(
            "ProtectedAtomDB requires a PROTECTED backend. UNPROTECTED backends need no wrapper and "
            "FORWARD federation is handled by RemoteAtomDB::AtomDBKeySensitive.");
    }
...
void ProtectedAtomDB::raise_public_key_required(const string& method_name) {
    RAISE_ERROR("ProtectedAtomDB::" + method_name +
                "() is unavailable in protected AtomDBs. Use AtomDBKeySensitive passing a PublicKey.");
}

bool ProtectedAtomDB::ensure_registered(const atomdb_api_types::PublicKey& public_key) {
    if (!public_key.is_single_key()) {
        RAISE_ERROR("ProtectedAtomDB accepts only a single-key PublicKey");
    }
