 ...
 * - overloads without public_key: reject the call (protected access requires a key)
 * - overloads with public_key: authorize and delegate to the backend
 *
 */
class ProtectedAtomDB : public AtomDB {
   public:
    ...
    shared_ptr<Atom> get_atom(const string& handle, const string& public_key);

    shared_ptr<Node> get_node(const string& handle) override;
    shared_ptr<Node> get_node(const string& handle, const string& public_key);

    shared_ptr<Link> get_link(const string& handle) override;
    shared_ptr<Link> get_link(const string& handle, const string& public_key);

    vector<shared_ptr<Atom>> get_matching_atoms(bool is_toplevel, Atom& key) override;
    vector<shared_ptr<Atom>> get_matching_atoms(bool is_toplevel, Atom& key, const string& public_key);
    ...
    bool atom_exists(const string& handle, const string& public_key);

    bool node_exists(const string& handle) override;
    bool node_exists(const string& handle, const string& public_key);

    bool link_exists(const string& handle) override;
    bool link_exists(const string& handle, const string& public_key);

    set<string> atoms_exist(const vector<string>& handles) override;
    set<string> atoms_exist(const vector<string>& handles, const string& public_key);

    set<string> nodes_exist(const vector<string>& handles) override;
    set<string> nodes_exist(const vector<string>& handles, const string& public_key);
    ...
    bool delete_node(const string& handle, const string& public_key, bool delete_link_targets = false);

    bool delete_link(const string& handle, bool delete_link_targets = false) override;
    bool delete_link(const string& handle, const string& public_key, bool delete_link_targets = false);
    ...
    void re_index_patterns(const string& public_key, bool flush_patterns = true);

    size_t node_count() const override;
    size_t node_count(const string& public_key) const;

    size_t link_count() const override;
    size_t link_count(const string& public_key) const;

    size_t atom_count() const override;
    size_t atom_count(const string& public_key) const;
