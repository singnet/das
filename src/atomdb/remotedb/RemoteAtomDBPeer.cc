...
void RemoteAtomDBPeer::set_atom_resolver(AtomResolver resolver) { atom_resolver_ = std::move(resolver); }

void RemoteAtomDBPeer::refresh_cache_locked(const shared_ptr<Atom>& atom) {
    if (!atom) return;
    // Handle is content-addressed without custom attributes; replace any stale cached copy.
...
shared_ptr<HandleSet> RemoteAtomDBPeer::query_for_pattern(const LinkSchema& link_schema) {
...

    {
        ...
        if (schema_already_fetched(link_schema)) {
            LOG_DEBUG(
                "[RemoteDB(" << uid_ << ")] query_for_pattern(" << link_schema.handle() << ") cache-hit"
...
void RemoteAtomDBPeer::persist_atoms_to_local(const vector<shared_ptr<Atom>>& atoms) {
    ...
    while (!missing_targets_queue.empty()) {
        ...
        if (!fetched_atom && atom_resolver_) {
            fetched_atom = atom_resolver_(target_handle);
        }
        if (!fetched_atom) {
            LOG_ERROR("[RemoteDB(" << uid_ << ")] persist_atoms_to_local: unresolved target handle "
                                   << target_handle
                                   << " (not in local_persistence, peer atomdb, or federation)");
            continue;
        }
...
void RemoteAtomDBPeer::release_cache(bool persist_to_local, bool persist_entire_cache) {
...

    {
        ...
        if (persist_to_local && local_persistence_ && cache_->atom_count() > 0) {
            auto cached_atoms = cache_->get_all_atoms();
