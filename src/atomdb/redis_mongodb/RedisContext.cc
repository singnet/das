...
void RedisContext::append_command(const char* command) {
    if (cluster_flag) {
        Utils::error("The `append_command` is not supported in synchronous cluster mode.");
    } else {
...
void RedisContext::flush_commands() {
    if (cluster_flag) {
        Utils::error("The `flush_commands` command is not supported in synchronous cluster mode.");
    } else {
        redisReply* reply;
