...
MongodbAccessPermissionDocument::MongodbAccessPermissionDocument(const nlohmann::json& document) {
    ...
    this->public_key_ = document["public_key"].get<string>();
    this->full_access_ = document["full_access"].get<bool>();
    ...
    for (const auto& item : document["allowed_schemas"]) {
        ...
        this->entries_.emplace_back(tokens, item["read"].get<bool>(), item["write"].get<bool>());
    }
}

const string& MongodbAccessPermissionDocument::public_key() const { return this->public_key_; }

bool MongodbAccessPermissionDocument::full_access() const { return this->full_access_; }

const vector<AccessPermissionEntry>& MongodbAccessPermissionDocument::entries() const {
