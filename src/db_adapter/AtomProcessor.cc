...
AtomProcessor::~AtomProcessor() {
    if (!this->atoms.empty()) {
        LOG_INFO("AtomProcessor destructor called with " << this->atoms.size() << " unprocessed atoms. Persisting to AtomDB...");
...
void AtomProcessor::process_atoms(vector<Atom*> atoms) {
    ...
    if (this->atoms.size() >= 10000) {
        LOG_INFO("AtomProcessor batch size reached. Persisting " << this->atoms.size() << " atoms to AtomDB...");
        this->atomdb->add_atoms(this->atoms, false, true);
        for (auto& atom : this->atoms) {
            delete atom;
        }
