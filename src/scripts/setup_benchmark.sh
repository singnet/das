...
main() {
    ...
    echo -en "${YELLOW}loading a scenario…${RESET}"
    load_scenario_definition
    ret=$?
    if [[ $ret -eq 0 ]]; then
        echo -e "\r\033[K${GREEN}loading a scenario…OK${RESET}"
        print_scenario
    else
        echo -e "\r\033[K${RED}loading a scenario…FAILED (exit code $ret)${RESET}"
        exit $ret
    fi
