...
Wildcard::Wildcard(const string& type, const Properties& custom_attributes) : Atom(type, custom_attributes) {
    this->validate();
}
...
bool Wildcard::operator==(const Wildcard& other) {
...
string Wildcard::to_string() const {
    return "Wildcard(type: '" + this->type + "')";
}

string Wildcard::schema_handle() const {
