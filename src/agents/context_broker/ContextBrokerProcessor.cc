...
void ContextBrokerProcessor::create_context(shared_ptr<StoppableThread> monitor,
    ...
    if (!this->read_cache(proxy)) {
        ...
        pattern_proxy->parameters[BaseQueryProxy::ATTENTION_UPDATE] = (unsigned int) proxy->parameters.get_or(BaseQueryProxy::ATTENTION_UPDATE, (unsigned int) BaseQueryProxy::NONE);
        pattern_proxy->parameters[BaseQueryProxy::ATTENTION_CORRELATION] = (unsigned int) proxy->parameters.get_or(BaseQueryProxy::ATTENTION_CORRELATION, (unsigned int) BaseQueryProxy::NONE);
