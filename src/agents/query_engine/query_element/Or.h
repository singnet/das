    ...
    unsigned int select_answer() {
        ...
        for (unsigned int i = 0; i < N; i++) {
            if (this->next_input_to_process[i] < this->query_answer[i].size()) {
                ...
                if ((this->query_answer[i][this->next_input_to_process[i]]->importance > best_importance) || ((this->query_answer[i][this->next_input_to_process[i]]->importance == best_importance) && Utils::flip_coin())) {
