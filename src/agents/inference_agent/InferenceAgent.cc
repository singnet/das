...
void InferenceAgent::run() {
    ...
    while (true) {
        ...
        if (!inference_request_queue.empty()) {
            ...
            //         LOG_DEBUG("Iterator ID: " << inference_iterators[i]->get_local_id() << "
            //         finished"); send_stop_link_creation_request(
...
void InferenceAgent::send_link_creation_request(shared_ptr<InferenceRequest> inference_request) {
    this->inference_proxy_map[inference_request->get_id()] = vector<shared_ptr<LinkCreationRequestProxy>>();
    ...
    for (auto& request_iterator : inference_request->get_requests()) {
        ...
        this->inference_proxy_map[inference_request->get_id()].push_back(link_creation_proxy);
...
void InferenceAgent::send_stop_link_creation_request(shared_ptr<InferenceRequest> inference_request) {
    for (auto& proxy : this->inference_proxy_map[inference_request->get_id()]) {
        proxy->set_parameter(LinkCreationRequestProxy::Parameters::ABORT_FLAG, false);
...
void InferenceAgent::process_inference_abort_request(const string& request_id) {
    ...
    if (iterator_link_creation_request_map.find(request_id) !=
        iterator_link_creation_request_map.end()) {
        send_stop_link_creation_request(iterator_link_creation_request_map[request_id]);
        iterator_link_creation_request_map.erase(request_id);
    } else {
