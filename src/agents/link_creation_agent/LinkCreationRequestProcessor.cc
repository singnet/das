...
LinkCreationRequestProcessor::LinkCreationRequestProcessor() : BusCommandProcessor({ServiceBus::LINK_CREATE_REQUEST}) {
...
void LinkCreationRequestProcessor::process_link_create_request(shared_ptr<BusCommandProxy> proxy) {
    ...
    try{
        ...
        request.push_back(proxy->peer_id() + to_string(proxy->get_serial())); // Add request ID to the end of the request
        ...
        request_proxy->raise_error_on_peer("Error processing link creation request: " + string(e.what()));
        return;
    }
}
