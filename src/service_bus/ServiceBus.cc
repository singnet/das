...
void ServiceBus::forward_bus_command(shared_ptr<BusCommandProxy> proxy,
    ...
    lock_guard<mutex> semaphore(this->api_mutex);
    LOG_DEBUG(bus_node->node_id() << " is forwarding BUS command <" << proxy->command
                                  << "> for requestor " << requestor_id << " to "
                                  << requestor_proxy_node_id);
    if (proxy->issued) {
        RAISE_ERROR("Attempt to issue the same proxy twice");
    }
    if (requestor_id.empty() || requestor_proxy_node_id.empty()) {
        RAISE_ERROR("Forwarded bus command requires non-empty requestor_id and requestor_proxy_node_id");
    }
    proxy->issued = true;
    proxy->requestor_id = requestor_id;
    proxy->serial = this->next_request_serial++;
    proxy->pack_command_line_args();
    vector<string> args;
