...
static ListOfAny pylist_to_composite_type(const nb::list &py_list) {
  ...
  for (const auto &element : py_list) {
    ...
    if (type_name == "str") {
      ...
      ct_list.push_back(
          move(pylist_to_composite_type(nb::cast<nb::list>(element))));
    } else {
      throw invalid_argument("Invalid composite type element.");
    }
  }
...
static nb::dict link_to_dict(const Link &self) {
  nb::dict dict = atom_to_dict(self);
  dict[FieldNames::COMPOSITE_TYPE] =
      composite_type_to_pylist(self.composite_type);
...
static NodeTuple node_to_tuple(const Node &node) {
  return std::make_tuple(node._id, node.handle, node.composite_type_hash,
                         node.named_type, node.name, node.custom_attributes);
}
...
static LinkTuple link_to_tuple(const Link &link) {
  return std::make_tuple(link._id, link.handle, link.composite_type_hash,
                         link.named_type,
                         composite_type_to_pylist(link.composite_type),
                         link.named_type_hash, link.targets, link.is_toplevel,
                         link.custom_attributes, link.targets_documents);
}
 ...
 */
static void
init_link(Link &self, const string &_id, const string &handle,
          const string &composite_type_hash, const string &named_type,
          const nb::list &composite_type, const string &named_type_hash,
          ...
          const Link::TargetsDocuments &targets_documents =
              Link::TargetsDocuments{}) {
  new (&self) Link(_id, handle, composite_type_hash, named_type,
                   pylist_to_composite_type(composite_type), named_type_hash,
                   targets, is_toplevel, custom_attributes, targets_documents);
}
