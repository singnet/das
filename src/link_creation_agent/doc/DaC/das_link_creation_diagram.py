...
with Diagram("DAS Link Creation", filename="doc/assets/das_link_creation_hla", show=False):
    client = Person(name="DAS Server", description="Sends a Link creation request")

    with SystemBoundary("DAS Link Creation Agent"):
        das_node_server = Container(
            name="DAS Node Server",
            technology="DAS NODE",
            description="Receives a link creation request",
        )
        link_creation_agent = Container(
            name="DAS LCA",
            technology="C++ Core",
            description="Process a link creation request",
        )
        link_creation_agent_service = Container(
            name="DAS LCA",
            technology="C++ Service",
            description="Process multiple queries",
        )
        das_node_client = Container(
            name="DAS Node Client",
            technology="DAS NODE",
            description="Sends atoms creation requests",
        )
        das_node_client2 = Container(
            name="DAS Node Client",
            technology="DAS NODE",
            description="Sends query requests",
        )

    das_query_agent = Container(
        name="DAS Query Agent",
        technology="DAS NODE",
        description="Process Template Queries",
    )
