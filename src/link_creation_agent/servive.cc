...
LinkCreationService::LinkCreationService(int thread_count)
    : thread_pool(thread_count) {}

LinkCreationService::~LinkCreationService() {}

void LinkCreationService::process_request(
  ...
  auto job = [this, iterator, das_client, link_template, max_query_answers]() {
    ...
    while (!iterator->finished()) {
      ...
      if ((query_answer = iterator->pop()) == NULL) {
        cout << "LinkCreationService::process_request: Waiting for query answer"
             << endl;
        Utils::sleep();
      } else {
        cout << "LinkCreationService::process_request: Processing query_answer"
             << endl;
    ...
    cout << "LinkCreationService::process_request: Finished processing iterator"
         << endl;
  };
