...
static int string_to_int(const std::string &s) {
  if (!is_number(s)) {
    throw std::invalid_argument(
        "Can not convert string to int: Invalid arguments");
  }
  return std::stoi(s);
}
// TODO move this to a utils file
static std::string get_token(std::vector<std::string> &link_template,
                             int cursor) {
...
CustomField::CustomField(std::vector<std::string> &custom_fields) {
  if (get_token(custom_fields, 0) != "CUSTOM_FIELD")
    throw std::invalid_argument(
        "Can not create Custom Field: Invalid arguments");
  ...
  while (cursor < custom_fields.size()) {
    if (get_token(custom_fields, cursor) == "CUSTOM_FIELD") {
      ...
      int sub_custom_field_size =
          string_to_int(get_token(custom_fields, cursor + 2));
      std::string sub_custom_field_name = get_token(custom_fields, cursor + 1);
      custom_field_args.push_back(
          get_token(custom_fields, cursor)); // CUSTOM_FIELD
      custom_field_args.push_back(
          get_token(custom_fields, cursor + 1)); // field name
      custom_field_args.push_back(
          get_token(custom_fields, cursor + 2)); // field size
      cursor += 3;
      while (cursor < custom_fields.size()) {
      ...
      this->values.push_back(std::make_tuple(
          sub_custom_field_name, std::make_shared<CustomField>(custom_field)));
    } else {
      this->values.push_back(
          std::make_tuple(get_token(custom_fields, cursor),
                          get_token(custom_fields, cursor + 1)));
      cursor += 2;
    }
  }
}
...
std::vector<std::tuple<std::string, CustomFieldTypes>>
CustomField::get_values() {
  return this->values;
}

std::vector<std::string>
parse_sub_custom_field(std::vector<std::string> &link_template,
                       size_t &cursor) {
  if (get_token(link_template, cursor) != "CUSTOM_FIELD" ||
      link_template.size() < cursor + 3)
    throw std::invalid_argument(
        "Can not create Custom Field: Invalid arguments");
  std::vector<std::string> custom_field_args;
  ...
  custom_field_args.push_back(
      get_token(link_template, cursor + 1)); // field name
  custom_field_args.push_back(
      get_token(link_template, cursor + 2)); // field size
  cursor += 3;
...
std::vector<std::string>
parse_sub_link_template(std::vector<std::string> &link_template,
                        size_t &cursor) {
  if (get_token(link_template, cursor) != "LINK_CREATE" ||
      link_template.size() < cursor + 4)
    throw std::invalid_argument(
        "Can not create Link Template: Invalid arguments");
  int sub_link_template_size =
      string_to_int(get_token(link_template, cursor + 2));
  int sub_link_custom_field_size =
      string_to_int(get_token(link_template, cursor + 3));
  ...
  sub_link_template.push_back(
      get_token(link_template, cursor + 1)); // link type
  sub_link_template.push_back(
      get_token(link_template, cursor + 2)); // link size
  sub_link_template.push_back(
      get_token(link_template, cursor + 3)); // custom field size
  cursor += 4;
  while (cursor < link_template.size()) {
    if (sub_link_template_size == 0 && current_ptr == 0 &&
        sub_link_custom_field_size == 0 && custom_field_value_size == 0) {
      break;
    }
    ...
    if (sub_link_custom_field_size > 0 &&
        get_token(link_template, cursor) == "CUSTOM_FIELD") {
      current_ptr =
          3 + (string_to_int(get_token(link_template, cursor + 2)) * 2);
      sub_link_custom_field_size--;
    }
...
LinkCreateTemplate::LinkCreateTemplate(
    std::vector<std::string> &link_template) {
  int starting_pos = 0;
  ...
  std::size_t link_template_size =
      string_to_int(get_token(link_template, starting_pos + 1));
  std::size_t custom_field_size =
      string_to_int(get_token(link_template, starting_pos + 2));
  while (cursor < link_template.size()) {
    if (get_token(link_template, cursor) == "NODE") {
      Node node;
      ...
      std::vector<std::string> sub_link_template =
          parse_sub_link_template(link_template, cursor);
      LinkCreateTemplateTypes sub_link =
          std::make_shared<LinkCreateTemplate>(sub_link_template);
      this->targets.push_back(sub_link);
    } else if (get_token(link_template, cursor) == "CUSTOM_FIELD") {
      std::vector<std::string> sub_custom_field =
          parse_sub_custom_field(link_template, cursor);
  ...
  if (this->targets.size() != link_template_size ||
      this->custom_fields.size() != custom_field_size) {
    throw std::invalid_argument(
        "Can not create Link Template: Invalid arguments");
  }
}
...
std::vector<CustomField> LinkCreateTemplate::get_custom_fields() {
...
std::string LinkCreateTemplate::to_string() {
  ...
  for (auto target : this->targets) {
    ...
    } else if (std::holds_alternative<std::shared_ptr<LinkCreateTemplate>>(
                   target)) {
...
std::string CustomField::to_string() {
  std::string custom_field = "CUSTOM_FIELD " + this->name + " " +
