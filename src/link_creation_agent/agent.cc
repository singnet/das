...
LinkCreationAgent::LinkCreationAgent(string config_path) {
  ...
  load_config();
  link_creation_node_server =
      new LinkCreationNode(link_creation_agent_server_id);
...
void LinkCreationAgent::run() {
  ...
  while (true) {
    ...
    if (!link_creation_node_server->is_query_empty()) {
      ...
      if (lca_request != NULL &&
          (lca_request->infinite || lca_request->repeat > 0)) {
      ...
      if (!request_buffer.empty()) {
        ...
        current_buffer_position =
            current_buffer_position % request_buffer.size();
    ...
    shared_ptr<RemoteIterator<HandlesAnswer>> iterator =
        query(lca_request->query, lca_request->context,
    ...
    if (lca_request->infinite || lca_request->repeat > 0) {
      ...
      if (is_from_buffer) {
        request_buffer.erase(request_buffer.begin() + current_buffer_position -
                             1);
      } else {
        delete lca_request;
      }
...
shared_ptr<RemoteIterator<HandlesAnswer>>
LinkCreationAgent::query(vector<string> &query_tokens, string context,
                         bool update_attention_broker) {
  return shared_ptr<RemoteIterator<HandlesAnswer>>(
      query_node_client->pattern_matcher_query(query_tokens, context,
...
LinkCreationAgentRequest *
LinkCreationAgent::create_request(vector<string> request) {
  try {
