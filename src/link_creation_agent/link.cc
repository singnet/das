...
vector<string> Link::get_targets() { return this->targets; }

void Link::set_type(string type) { this->type = type; }

void Link::set_targets(vector<string> targets) { this->targets = targets; }

void Link::add_target(string target) { this->targets.push_back(target); }

vector<string> Link::tokenize() { return targets; }

Link Link::untokenize(string link) { return Link(); }
