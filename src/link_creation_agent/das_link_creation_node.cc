...
LinkCreationNode::LinkCreationNode(const string &node_id,
...
void LinkCreationNode::add_request(vector<string> request) {
  shared_queue.enqueue(request);
}

shared_ptr<Message> LinkCreationNode::message_factory(string &command,
                                                      vector<string> &args) {
