...
TEST_F(RedisMongoDBTest, AddAtoms) {
    vector<Atom*> atoms;
    ...
    for (int j = 0; j < 5; j++) {
        for (int i = 0; i < num_atoms; i++) {
            auto similarity = new Node("Symbol", "Similarity-1-" + to_string(i));
            auto node_1 = new Node("Symbol", "add-atoms-1-" + to_string(i));
            auto node_2 = new Node("Symbol", "add-atoms-2-" + to_string(i));
            auto link = new Link("Expression", {similarity->handle(), node_1->handle(), node_2->handle()});

            atoms.push_back(similarity);
            atoms.push_back(node_1);
            atoms.push_back(node_2);
            atoms.push_back(link);
        }
        cout << "========> Adding atoms... " << atoms.size() << endl;

        auto handles = db->add_atoms(atoms);
        EXPECT_EQ(handles.size(), 4 * num_atoms);

        auto links_exist = db->atoms_exist(handles);
        EXPECT_EQ(links_exist.size(), 4 * num_atoms);
        atoms.clear();
    }

    

    // EXPECT_EQ(db->delete_atoms(handles), 40);

    // auto atoms_exist_after_delete = db->atoms_exist(handles);
    // EXPECT_EQ(atoms_exist_after_delete.size(), 0);
}
