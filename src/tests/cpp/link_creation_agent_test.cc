...
TEST_F(LinkCreationAgentTest, TestRequest) {
  // Simulate a request
  vector<string> request = {"query1", "LINK_CREATE",  "test", "1",
                            "0",      "VARIABLE",     "V1",   "10",
                            "5",      "test_context", "true"};
  LinkCreationAgentRequest *lca_request =
      LinkCreationAgent::create_request(request);
...
TEST(LinkCreateTemplate, TestCustomField) {
  vector<string> args = {"CUSTOM_FIELD", "field1", "2",     "N1",
                         "value1",       "N2",     "value2"};
  ...
  vector<string> args_multiple = {"CUSTOM_FIELD", "multi_field", "3",
                                  "N1",           "value1",      "N2",
                                  "value2",       "N3",          "value3"};
  CustomField custom_field_multiple(args_multiple);
  EXPECT_EQ(custom_field_multiple.get_name(), "multi_field");
  vector<tuple<string, CustomFieldTypes>> values_multiple =
      custom_field_multiple.get_values();
  ...
  vector<string> args_nested = {
      "CUSTOM_FIELD", "nested_field", "2", "N1", "value1",
      "CUSTOM_FIELD", "inner_field",  "1", "N2", "value2"};
  CustomField custom_field_nested(args_nested);
  EXPECT_EQ(custom_field_nested.get_name(), "nested_field");
  vector<tuple<string, CustomFieldTypes>> values_nested =
      custom_field_nested.get_values();
  EXPECT_EQ(values_nested.size(), 2);
  EXPECT_EQ(get<string>(get<1>(values_nested[0])), "value1");
  shared_ptr<CustomField> inner_field =
      get<shared_ptr<CustomField>>(get<1>(values_nested[1]));
  EXPECT_EQ(inner_field->get_name(), "inner_field");
  vector<tuple<string, CustomFieldTypes>> inner_values =
      inner_field->get_values();
...
TEST(LinkCreateTemplate, TestLinkCreateTemplate) {
  ...
  */
  string link_template_str = "LINK_CREATE Similarity 3 1 VARIABLE V1 "
  ...
  link_template_str = "LINK_CREATE I 3 0 VARIABLE V1 LINK_CREATE Test 3 0 NODE "
  ...
  link_template_str = "LINK_CREATE Similarity 2 1 VARIABLE V1 VARIABLE V2 "
  ...
  link_template_str = "LINK_CREATE link_type 2 1 NODE type1 value1 VARIABLE "
  ...
  EXPECT_EQ(lct5.to_string(), "LINK_CREATE TestLink 4 1 NODE NodeType1 Value1 "
  ...
  link_template_str = "LINK_CREATE AnotherLink 2 1 VARIABLE VarA NODE "
  ...
  EXPECT_EQ(get<string>(get<1>(lct6.get_custom_fields()[0].get_values()[0])),
            "ValueA");
  ...
  EXPECT_EQ(get<string>(get<1>(lct7.get_custom_fields()[0].get_values()[0])),
            "Val1");
  EXPECT_EQ(get<0>(lct7.get_custom_fields()[0].get_values()[1]), "N2");
  EXPECT_EQ(get<string>(get<1>(lct7.get_custom_fields()[0].get_values()[1])),
            "Val2");
  ...
  EXPECT_EQ(get<string>(get<1>(lct7.get_custom_fields()[1].get_values()[0])),
            "Val3");
  link_template.clear();
  link_template_str.clear();

  link_template_str = "LINK_CREATE SimpleLink 2 0 VARIABLE SimpleVar NODE "
...
TEST(LinkCreateTemplate, TestInvalidTargetCount) {
  string link_template_str =
      "LINK_CREATE Similarity 3 1 VARIABLE V1 NODE Symbol A";
  auto link_template = split(link_template_str, ' ');
  EXPECT_THROW(LinkCreateTemplate lct(link_template), invalid_argument);
}

TEST(LinkCreateTemplate, TestInvalidCustomField) {
  string link_template_str = "LINK_CREATE Similarity 2 1 VARIABLE V1 NODE "
...
TEST(LinkCreateTemplate, TestInvalidVariable) {
  string link_template_str =
      "LINK_CREATE Similarity 2 1 VARIABLE NODE Symbol A";
  auto link_template = split(link_template_str, ' ');
  EXPECT_THROW(LinkCreateTemplate lct(link_template), invalid_argument);
}
...
TEST(LinkCreateTemplate, TestInvalidNode) {
  string link_template_str =
      "LINK_CREATE Similarity 2 1 VARIABLE V1 NODE Symbol";
  auto link_template = split(link_template_str, ' ');
  EXPECT_THROW(LinkCreateTemplate lct(link_template), invalid_argument);
}
