...
public:
  ...
  TestNode(const string &node_id, const string &server_id,
           LeadershipBrokerType leadership_algorithm,
           MessageBrokerType messaging_backend, bool is_server)
      : DistributedAlgorithmNode(node_id, leadership_algorithm,
                                 messaging_backend) {
  ...
  std::shared_ptr<Message> message_factory(string &command,
                                           vector<string> &args) {
...
TEST(DistributedAlgorithmNode, basics) {
  ...
  for (auto messaging_type :
       {MessageBrokerType::RAM, MessageBrokerType::GRPC}) {
    server = new TestNode(server_id, server_id,
                          LeadershipBrokerType::SINGLE_MASTER_SERVER,
                          messaging_type, true);
    client1 = new TestNode(client1_id, server_id,
                           LeadershipBrokerType::SINGLE_MASTER_SERVER,
                           messaging_type, false);
    client2 = new TestNode(client2_id, server_id,
                           LeadershipBrokerType::SINGLE_MASTER_SERVER,
                           messaging_type, false);
...
TEST(DistributedAlgorithmNode, communication) {
  ...
  for (auto messaging_type :
    ...
    EXPECT_EQ(client2->node_joined_network_count,
              0); // TODO Fix this. This count should be 2
