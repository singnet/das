...
TEST_F(RemoteAtomDBPeerTest, PersistsToLocal) {
    ...
    // Local persistence should have it
    EXPECT_TRUE(local_->node_exists(human_handle));
    auto from_local = local_->get_node(human_handle);
    ASSERT_NE(from_local, nullptr);
    EXPECT_EQ(from_local->handle(), human_handle);
}
...
TEST_F(RemoteAtomDBTest, AllowNestedIndexing) { EXPECT_FALSE(db_->allow_nested_indexing()); }
