...
void check_query(vector<string> &query, unsigned int expected_count,
                 DASNode *das, DASNode *requestor, const string &context) {
  cout << "XXXXXXXXXXXXXXXX DASNode.queries CHECK BEGIN" << endl;
  QueryAnswer *query_answer;
  RemoteIterator<HandlesAnswer> *response =
      requestor->pattern_matcher_query(query, context);
...
TEST(DASNode, queries) {
  ...
  vector<string> q1 = {"LINK_TEMPLATE", "Expression", "3",        "NODE",
                       "Symbol",        "Similarity", "VARIABLE", "v1",
                       "VARIABLE",      "v2"};
  int q1_expected_count = 14;

  vector<string> q2 = {"LINK_TEMPLATE", "Expression", "3",    "NODE",
                       "Symbol",        "Similarity", "NODE", "Symbol",
                       "\"human\"",     "VARIABLE",   "v1"};
  int q2_expected_count = 3;

  vector<string> q3 = {
      "AND",  "2",      "LINK_TEMPLATE", "Expression",    "3",
  ...
  vector<string> q5 = {
      "OR",   "2",      "LINK_TEMPLATE", "Expression",    "3",
      "NODE", "Symbol", "Similarity",    "VARIABLE",      "v1",
      "NODE", "Symbol", "\"human\"",     "LINK_TEMPLATE", "Expression",
