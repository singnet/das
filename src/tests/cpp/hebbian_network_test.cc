...
TEST(HebbianNetwork, stress) {
  ...
  unsigned int num_insertions =
      (handle_space_size * 2) * (handle_space_size * 2);
...
bool visit2(HandleTrie::TrieNode *node, HebbianNetwork::Node *source,
            forward_list<HebbianNetwork::Node *> &targets,
            unsigned int targets_size, ImportanceType sum_weights, void *data) {
