...
public:
  ...
  void add(const char *handle, double importance,
           const array<const char *, 1> &labels,
           const array<const char *, 1> &values, bool sleep_flag = true) {
...
void check_query_answer(string tag, HandlesAnswer *query_answer,
                        double importance, unsigned int handles_size,
...
TEST(AndOperator, basics) {
  ...
  EXPECT_FALSE(sink.finished());
  EXPECT_FALSE((query_answer = dynamic_cast<HandlesAnswer *>(sink.pop())) ==
               NULL);
  EXPECT_TRUE(sink.empty());
  EXPECT_FALSE(sink.finished());
...
TEST(AndOperator, operation_logic) {
  ...
  for (unsigned int clause = 0; clause < clause_count; clause++) {
    ...
    std::sort(std::begin(importance[clause]), std::end(importance[clause]),
              std::greater<double>{});
  }
  ...
  for (unsigned int clause = 0; clause < clause_count; clause++) {
    for (unsigned int link = 0; link < link_count; link++) {
      source[clause]->add(random_handle().c_str(), importance[clause][link],
                          {"v"}, {"1"}, false);
    }
  ...
  for (unsigned int i = 0; i < link_count; i++) {
    for (unsigned int j = 0; j < link_count; j++) {
      for (unsigned int k = 0; k < link_count; k++) {
        pair.importance = importance[0][i] > importance[1][j]
                              ? importance[0][i]
                              : importance[1][j];
        pair.importance = importance[2][k] > pair.importance ? importance[2][k]
                                                             : pair.importance;
  ...
  while (!(sink->empty() && sink->finished())) {
    ...
    cout << count << " CHECK: " << query_answer->importance << " "
         << pair.importance << " (" << pair.fitness << ")" << endl;
