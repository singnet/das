...
sleep 5
make run-busnode OPTIONS="--service=query-engine --config=/opt/das/config/das.json" >> /tmp/qa.log 2>&1 &
sleep 5
make run-busnode OPTIONS="--service=evolution-agent --bus-endpoint=localhost:40002 --config=/opt/das/config/das.json" >> /tmp/ev.log 2>&1 &
sleep 5
