...
QueryNode<AnswerType>::QueryNode(const string &node_id, bool is_server,
                                 MessageBrokerType messaging_backend)
    : DistributedAlgorithmNode(node_id,
...
template <class AnswerType> QueryNode<AnswerType>::~QueryNode() {}

template <class AnswerType> void QueryNode<AnswerType>::graceful_shutdown() {
  if (is_shutting_down()) {
    return;
  }
...
template <class AnswerType> bool QueryNode<AnswerType>::is_shutting_down() {
  bool answer;
  this->shutdown_flag_mutex.lock();
...
template <class AnswerType>
shared_ptr<Message>
QueryNode<AnswerType>::message_factory(string &command, vector<string> &args) {
...
QueryNodeServer<AnswerType>::QueryNodeServer(
    const string &node_id, MessageBrokerType messaging_backend)
    : QueryNode<AnswerType>(node_id, true, messaging_backend) {
  this->join_network();
  this->query_answer_processor = new thread(
      &QueryNodeServer<AnswerType>::query_answer_processor_method, this);
}

template <class AnswerType> QueryNodeServer<AnswerType>::~QueryNodeServer() {
...
void QueryNodeClient<AnswerType>::query_answer_processor_method() {
  ...
  while (!this->is_shutting_down()) {
    while ((query_answer = (QueryAnswer *)this->query_answer_queue.dequeue()) !=
           NULL) {
...
QueryNodeClient<AnswerType>::QueryNodeClient(
    const string &node_id, const string &server_id,
    MessageBrokerType messaging_backend)
    : QueryNode<AnswerType>(node_id, true, messaging_backend) {
  this->query_answer_processor = new thread(
      &QueryNodeClient<AnswerType>::query_answer_processor_method, this);
  this->server_id = server_id;
  this->add_peer(server_id);
  this->join_network();
}

template <class AnswerType> QueryNodeClient<AnswerType>::~QueryNodeClient() {
  this->graceful_shutdown();
  if (this->query_answer_processor != NULL) {
    this->query_answer_processor->join();
    this->query_answer_processor = NULL;
...
template <class AnswerType>
QueryAnswerFlow<AnswerType>::QueryAnswerFlow(string command,
                                             vector<string> &args) {
...
template <class AnswerType>
QueryAnswerTokensFlow<AnswerType>::QueryAnswerTokensFlow(string command,
                                                         vector<string> &args) {
...
template <class AnswerType>
QueryAnswersFinished<AnswerType>::QueryAnswersFinished(string command,
                                                       vector<string> &args) {}
