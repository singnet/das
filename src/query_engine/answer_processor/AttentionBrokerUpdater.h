...
public:
  AttentionBrokerUpdater(const string &query_context = "")
      : attention_broker_address(ATTENTION_BROKER_ADDRESS),
        queue_processor_finished(false), query_context(query_context),
        queue_processor_method(
            new thread(&AttentionBrokerUpdater::queue_processor, this)) {}
...
protected:
  void queue_processor() {
    ...
    unordered_map<string, unsigned int>
        joint_answer; // Auxiliary joint count of handles.
    // HandleTrie *joint_answer = new HandleTrie(HANDLE_HASH_SIZE - 1);

    // Protobuf data structures
    dasproto::HandleList
        *handle_list; // will contain single_answer (can't be used directly
    ...
    do {
      ...
      string handle;
      unsigned int count;
      while ((handles_answer =
                  (HandlesAnswer *)this->answers_queue.dequeue()) != NULL) {
