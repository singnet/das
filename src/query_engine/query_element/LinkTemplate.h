 ...
 */
template <unsigned int ARITY> class LinkTemplate : public Source {
public:
  // --------------------------------------------------------------------------------------------
  ...
  LinkTemplate(const string &type, const array<QueryElement *, ARITY> &targets,
    ...
    this->handle_keys[0] =
        (wildcard_flag ? (char *)AtomDB::WILDCARD.c_str()
    ...
    this->handle =
        shared_ptr<char>(composite_hash(this->handle_keys, ARITY + 1));
    if (!wildcard_flag) {
    ...
    this->id = this->handle.get() +
               std::to_string(LinkTemplate::next_instance_count());
  }
  ...
  virtual void setup_buffers() {
    ...
    if (this->inner_template.size() > 0) {
      ...
      case 4: {
        this->inner_template_iterator = make_shared<Iterator<HandlesAnswer>>(
            new And<4>({inner_template[0], inner_template[1], inner_template[2],
                        inner_template[3]}),
            true);
        break;
      }
      default: {
        Utils::error(
            "Invalid number of inner templates (> 4) in link template.");
      }
      }
    }
    this->local_buffer_processor =
        new thread(&LinkTemplate::local_buffer_processor_method, this);
    fetch_links();
#ifdef DEBUG
...
private:
  ...
  void get_importance(const dasproto::HandleList &handle_list,
    ...
    if (handle_list.list_size() <= MAX_GET_IMPORTANCE_BUNDLE_SIZE) {
      stub->get_importance(new grpc::ClientContext(), handle_list,
                           &importance_list);
      return;
    }
...
#endif
      stub->get_importance(new grpc::ClientContext(), small_handle_list,
  ...
  void fetch_links() {
    ...
    if (answer_count > 0) {
      ...
      this->atom_document =
          new shared_ptr<atomdb_api_types::AtomDocument>[answer_count];
      this->local_answers = new HandlesAnswer *[answer_count];
      this->next_inner_answer = new unsigned int[answer_count];
      for (unsigned int i = 0; i < answer_count; i++) {
        this->atom_document[i] =
            db->get_atom_document(this->fetch_result->get_handle(i));
        query_answer = new HandlesAnswer(this->fetch_result->get_handle(i),
      ...
      std::sort(fetched_answers.begin(), fetched_answers.end(),
                less_than_query_answer());
      for (unsigned int i = 0; i < answer_count; i++) {
        if (this->inner_template.size() == 0) {
  ...
  bool is_feasible(unsigned int index) {
    ...
    while (cursor < inner_answers_size) {
      if (this->inner_answers[cursor] != NULL) {
        ...
        if (passed_first_check && this->local_answers[index]->merge(
                                      this->inner_answers[cursor], false)) {
  ...
  bool ingest_newly_arrived_answers() {
    ...
    while ((query_answer = dynamic_cast<HandlesAnswer *>(
                this->inner_template_iterator->pop())) != NULL) {
  ...
  void local_buffer_processor_method() {
    if (this->inner_template.size() == 0) {
      while (!(this->is_flow_finished() && this->local_buffer.empty())) {
        HandlesAnswer *query_answer;
        while ((query_answer = (HandlesAnswer *)this->local_buffer.dequeue()) !=
               NULL) {
