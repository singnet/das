 ...
 *
 * And operates on N clauses. Each clause can be either a Source or another
 * Operator.
 */
template <unsigned int N> class And : public Operator<N> {
public:
  // --------------------------------------------------------------------------------------------
   ...
   * Constructor.
   *
   * @param clauses Array with N clauses (each clause is supposed to be a Source
   * or an Operator).
   */
...
private:
  ...
  public:
    ...
    CandidateRecord(const CandidateRecord &other) {
      ...
      memcpy((void *)this->index, (const void *)other.index,
             N * sizeof(unsigned int));
      memcpy((void *)this->answer, (const void *)other.answer,
  ...
  bool ready_to_process_candidate() {
    for (unsigned int i = 0; i < N; i++) {
      if ((!this->all_answers_arrived[i]) &&
          (this->query_answer[i].size() <=
           (this->next_input_to_process[i] + 1))) {
        return false;
      }
    }
    return true;
  ...
  void ingest_newly_arrived_answers() {
    ...
    for (unsigned int i = 0; i < N; i++) {
      while ((answer = dynamic_cast<HandlesAnswer *>(
                  this->input_buffer[i]->pop_query_answer())) != NULL) {
        no_new_answer = false;
  ...
  void expand_border(const CandidateRecord &last_used_candidate) {
    ...
    for (unsigned int new_candidate_count = 0; new_candidate_count < N;
         new_candidate_count++) {
      abort_candidate = false;
      candidate.fitness = 1.0;
      for (unsigned int answer_queue_index = 0; answer_queue_index < N;
           answer_queue_index++) {
    ...
    do {
      ...
      if (processed_all_input()) {
        ...
        if (all_finished_flag &&
            !this->output_buffer->is_query_answers_finished() &&
            // processed_all_input() is double-checked on purpose to avoid race
            // condition
            processed_all_input()) {
      ...
      if (this->border.size() == 0) {
        ...
        for (unsigned int i = 0; i < N; i++) {
          candidate.answer[i] =
              this->query_answer[i][this->next_input_to_process[i]],
