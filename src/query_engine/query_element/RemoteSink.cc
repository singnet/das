...
RemoteSink<AnswerType>::RemoteSink(
    ...
    : Sink<AnswerType>(precedent, "RemoteSink(" + precedent->id + ")",
                       delete_precedent_on_destructor, true),
      queue_processor(new thread(&RemoteSink::queue_processor_method, this)),
      query_answer_processors(move(query_answer_processors)) {}

template <class AnswerType> RemoteSink<AnswerType>::~RemoteSink() {
  graceful_shutdown();
}
...
template <class AnswerType> void RemoteSink<AnswerType>::graceful_shutdown() {
#ifdef DEBUG
...
void RemoteSink<AnswerType>::queue_processor_method() {
  ...
  do {
    ...
    bool idle_flag = true;
    QueryAnswer *qa;
    while ((qa = dynamic_cast<QueryAnswer *>(
                this->input_buffer->pop_query_answer())) != NULL) {
      for (const auto &processor : this->query_answer_processors) {
