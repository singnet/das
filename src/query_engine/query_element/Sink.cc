...
Sink<AnswerType>::Sink(QueryElement *precedent, const string &id,
...
template <class AnswerType> Sink<AnswerType>::~Sink() {
...
template <class AnswerType> void Sink<AnswerType>::setup_buffers() {
...
template <class AnswerType> void Sink<AnswerType>::graceful_shutdown() {
  this->input_buffer->graceful_shutdown();
