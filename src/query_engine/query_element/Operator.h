 ...
 * Operator or a Sink.
 */
template <unsigned int N> class Operator : public QueryElement {
public:
  // --------------------------------------------------------------------------------------------
  ...
  virtual void setup_buffers() {
    ...
    this->output_buffer = make_shared<QueryNodeClient<HandlesAnswer>>(
        this->id, this->subsequent_id);
    string server_node_id;
    for (unsigned int i = 0; i < N; i++) {
      server_node_id = this->id + "_" + to_string(i);
      this->input_buffer[i] =
          make_shared<QueryNodeServer<HandlesAnswer>>(server_node_id);
