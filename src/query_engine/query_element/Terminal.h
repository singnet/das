...
/**
 * A QueryElement which represents terminals (i.e. Nodes, Links and Variables)
 * in the query tree.
 */
...
public:
  ...
  /**
   * Empty implementation. There are no QueryNode element or local thread to
   * shut down.
   */
  void virtual graceful_shutdown() {}

  /**
   * Returns a string representation of this Terminal (mainly for debugging; not
   ...
   *
   * Actually, only Nodes and Variables have names; Links' name is an empty
   * string.
   */
  string name;
};
...
public:
  ...
  Node(const string &type, const string &name) : Terminal() {
    ...
    this->name = name;
    this->handle = shared_ptr<char>(
        terminal_hash((char *)type.c_str(), (char *)name.c_str()));
  }
 ...
 */
template <unsigned int ARITY> class Link : public Terminal {
public:
  /**
   * Constructor.
   *
   ...
   *         handles (i.e. strings). No nesting of Nodes or other Links are
   * allowed.
   */
  Link(const string &type, const array<QueryElement *, ARITY> &targets)
      : Terminal() {
    this->name = "";
    this->type = type;
    ...
    for (unsigned int i = 1; i < (ARITY + 1); i++) {
      if (targets[i - 1]->is_terminal &&
          !((Terminal *)targets[i - 1])->is_variable) {
