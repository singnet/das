 ...
 *
 * Or operates on N clauses. Each clause can be either a Source or another
 * Operator.
 */
template <unsigned int N> class Or : public Operator<N> {
public:
  // --------------------------------------------------------------------------------------------
  ...
  void or_operator_method() {
    do {
      ...
      HandlesAnswer *selected_query_answer =
          this->query_answer[selected_clause]
                            [this->next_input_to_process[selected_clause]++];
      cout << std::to_string(selected_clause) << ": "
           << selected_query_answer->to_string() << endl;
