...
bool Assignment::assign(const char *label, const char *value) {
  ...
  if (this->size == MAX_NUMBER_OF_VARIABLES_IN_QUERY) {
    Utils::error("Assignment size exceeds the maximal number of allowed "
...
bool Assignment::is_compatible(const Assignment &other) {
  for (unsigned int i = 0; i < this->size; i++) {
    for (unsigned int j = 0; j < other.size; j++) {
      if ((strncmp(this->labels[i], other.labels[j], MAX_VARIABLE_NAME_SIZE) ==
           0) &&
...
string Assignment::to_string() {
  ...
  for (unsigned int i = 0; i < this->size; i++) {
    answer +=
        "(" + string(this->labels[i]) + ": " + string(this->values[i]) + ")";
    if (i != (this->size - 1)) {
      answer += ", ";
    }
  }
...
HandlesAnswer *HandlesAnswer::copy(HandlesAnswer *base) { // Static method
  ...
  memcpy((void *)copy->handles, (const void *)base->handles,
    ...
    if (merge_handles) {
      ...
      for (unsigned int j = 0; j < other->handles_size; j++) {
        ...
        for (unsigned int i = 0; i < this->handles_size; i++) {
          if (strncmp(this->handles[i], other->handles[j], HANDLE_HASH_SIZE) ==
              0) {
            already_exist = true;
            break;
          }
...
string HandlesAnswer::to_string() {
  ...
  answer += "] " + this->assignment.to_string() + " " +
            std::to_string(this->importance);
  // answer += "] " + this->assignment.to_string();
  return answer;
}

const string &HandlesAnswer::tokenize() {
  // char_count is computed to be slightly larger than actually required by
  // assuming e.g. 3 digits to represent sizes
...
static inline void read_token(const char *token_string, unsigned int &cursor,
                              char *token, unsigned int token_size) {
...
void HandlesAnswer::untokenize(const string &tokens) {
  ...
  if (this->assignment.size > MAX_NUMBER_OF_VARIABLES_IN_QUERY) {
    Utils::error("Invalid number of assignments: " +
                 std::to_string(this->assignment.size) +
                 " untokenizing HandlesAnswer");
  }
  ...
  if (token_string[cursor] != '\0') {
    Utils::error(
        "Invalid token string - invalid text after HandlesAnswer definition");
  }
}
