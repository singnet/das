...
DASNode::DASNode(const string &node_id, const string &server_id)
...
void DASNode::initialize() {
  ...
  if (this->is_server) {
    ...
    this->next_query_port =
        (this->first_query_port + this->last_query_port) / 2;
  }
}
...
RemoteIterator<HandlesAnswer> *
DASNode::pattern_matcher_query(const vector<string> &tokens,
                               const string &context,
                               bool update_attention_broker) {
#ifdef DEBUG
  cout << "DASNode::pattern_matcher_query() BEGIN" << endl;
  cout << "DASNode::pattern_matcher_query() tokens.size(): " << tokens.size()
       << endl;
  cout << "DASNode::pattern_matcher_query() context: " << context << endl;
  cout << "DASNode::pattern_matcher_query() update_attention_broker: "
       << update_attention_broker << endl;
#endif
  if (this->is_server) {
  ...
  vector<string> args = {query_id, context,
                         std::to_string(update_attention_broker)};
...
int DASNode::count_query(const vector<string> &tokens, const string &context,
                         bool update_attention_broker) {
#ifdef DEBUG
  ...
  cout << "DASNode::count_query() update_attention_broker: "
       << update_attention_broker << endl;
#endif
  if (this->is_server) {
  ...
  while (not count_iterator->finished()) {
    if ((count_answer = dynamic_cast<CountAnswer *>(count_iterator->pop())) !=
        nullptr) {
...
#ifdef DEBUG
  cout << "DASNode::next_query_id(): "
       << this->local_host + ":" + std::to_string(port) << endl;
#endif
...
shared_ptr<Message> DASNode::message_factory(string &command,
                                             vector<string> &args) {
  std::shared_ptr<Message> message =
      DistributedAlgorithmNode::message_factory(command, args);
  if (message) {
    return message;
  }
...
QueryElement *PatternMatchingQuery::build_link_template(
    vector<string> &tokens, unsigned int cursor,
    stack<QueryElement *> &element_stack) {
  unsigned int arity = std::stoi(tokens[cursor + 2]);
  if (element_stack.size() < arity) {
    Utils::error("PatternMatchingQuery message: parse error in tokens - too "
  ...
  default: {
    Utils::error("PatternMatchingQuery message: max supported arity for "
                 "LINK_TEMPLATE: 10");
  }
  }
...
QueryElement *
PatternMatchingQuery::build_and(vector<string> &tokens, unsigned int cursor,
                                stack<QueryElement *> &element_stack) {
  ...
  default: {
    Utils::error(
        "PatternMatchingQuery message: max supported num_clauses for AND: 10");
  }
  }
  return NULL; // Just to avoid warnings. This is not actually reachable.
}
...
QueryElement *
PatternMatchingQuery::build_or(vector<string> &tokens, unsigned int cursor,
                               stack<QueryElement *> &element_stack) {
  unsigned int num_clauses = std::stoi(tokens[cursor + 1]);
  if (element_stack.size() < num_clauses) {
    Utils::error("PatternMatchingQuery message: parse error in tokens - too "
  ...
  default: {
    Utils::error(
        "PatternMatchingQuery message: max supported num_clauses for OR: 10");
  }
  }
  return NULL; // Just to avoid warnings. This is not actually reachable.
}
...
QueryElement *
PatternMatchingQuery::build_link(vector<string> &tokens, unsigned int cursor,
                                 stack<QueryElement *> &element_stack) {
  unsigned int arity = std::stoi(tokens[cursor + 2]);
  if (element_stack.size() < arity) {
    Utils::error("PatternMatchingQuery message: parse error in tokens - too "
  ...
  default: {
    Utils::error(
        "PatternMatchingQuery message: max supported arity for LINK: 10");
  }
  }
  return NULL; // Just to avoid warnings. This is not actually reachable.
}

PatternMatchingQuery::PatternMatchingQuery(string command,
  ...
  unsigned int cursor =
      3; // TODO XXX: change this when requestor is set in basic Message
  unsigned int tokens_count = tokens.size();

#ifdef DEBUG
  cout << "PatternMatchingQuery::PatternMatchingQuery() tokens_count: "
       << tokens_count << endl;
#endif
  ...
  while (!execution_stack.empty()) {
    ...
    if (tokens[cursor] == "NODE") {
      ...
      Utils::error("Invalid token " + tokens[cursor] +
...
void PatternMatchingQuery::act(shared_ptr<MessageFactory> node) {
  ...
  cout << "PatternMatchingQuery::act() this->requestor_id: "
       << this->requestor_id << endl;
#endif
  auto das_node = dynamic_pointer_cast<DASNode>(node);

  if (this->command == DASNode::PATTERN_MATCHING_QUERY ||
      this->command == DASNode::COUNTING_QUERY) {
    ...
    if (this->command == DASNode::PATTERN_MATCHING_QUERY) {
      query_answer_processors.push_back(
          make_unique<HandlesAnswerProcessor>(local_id, remote_id));
      if (this->update_attention_broker)
        query_answer_processors.push_back(
            make_unique<AttentionBrokerUpdater>(this->context));
    } else if (this->command == DASNode::COUNTING_QUERY) {
      query_answer_processors.push_back(
          make_unique<CountAnswerProcessor>(local_id, remote_id));
    }

    // TODO: eliminate this memory leak
    RemoteSink<HandlesAnswer> *remote_sink = new RemoteSink<HandlesAnswer>(
        this->root_query_element, move(query_answer_processors));
  } else {
    Utils::error("Invalid command " + this->command +
                 " in PatternMatchingQuery message");
  }
...
CountingQuery::CountingQuery(string command, vector<string> &tokens) {
  ...
  this->pattern_matching_query =
      make_unique<PatternMatchingQuery>(command, tokens);
