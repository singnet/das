...
void AtomDB::attention_broker_setup() {
  ...
  auto stub = dasproto::AttentionBroker::NewStub(grpc::CreateChannel(
      attention_broker_address, grpc::InsecureChannelCredentials()));
  status = stub->ping(&context, empty, &ack);
  if (status.ok()) {
    std::cout << "Connected to AttentionBroker at " << attention_broker_address
              << endl;
  } else {
    Utils::error("Couldn't connect to AttentionBroker at " +
...
void AtomDB::redis_setup() {
  ...
  if (host == "" || port == "") {
    Utils::error("You need to set Redis access info as environment variables: "
                 "DAS_REDIS_HOSTNAME, "
...
void AtomDB::mongodb_setup() {
  ...
  if (host == "" || port == "" || user == "" || password == "") {
    Utils::error(
        string(
            "You need to set MongoDB access info as environment variables: ") +
        "DAS_MONGODB_HOSTNAME, DAS_MONGODB_PORT, DAS_MONGODB_USERNAME and "
  ...
  try {
    ...
    // std::cout << "Connected to MongoDB at " << address << " Atom count: " <<
    // atom_count << endl;
...
shared_ptr<atomdb_api_types::HandleList>
AtomDB::query_for_pattern(shared_ptr<char> pattern_handle) {
  redisReply *reply = (redisReply *)redisCommand(
      this->redis_single, "SMEMBERS %s:%s", REDIS_PATTERNS_PREFIX.c_str(),
      pattern_handle.get());
  if (reply == NULL) {
  ...
  // NOTE: Intentionally, we aren't destroying 'reply' objects.'reply' objects
  // are destroyed in ~RedisSet().
  return shared_ptr<atomdb_api_types::RedisSet>(
      new atomdb_api_types::RedisSet(reply));
}

shared_ptr<atomdb_api_types::HandleList>
AtomDB::query_for_targets(shared_ptr<char> link_handle) {
  return query_for_targets(link_handle.get());
}

shared_ptr<atomdb_api_types::HandleList>
AtomDB::query_for_targets(char *link_handle_ptr) {
  redisReply *reply =
      (redisReply *)redisCommand(this->redis_single, "GET %s:%s",
                                 REDIS_TARGETS_PREFIX.c_str(), link_handle_ptr);
  /*
  if (reply == NULL) {
      Utils::error("Redis error");
  }
...
shared_ptr<atomdb_api_types::AtomDocument>
AtomDB::get_atom_document(const char *handle) {
  ...
  auto reply = mongodb_collection.find_one(
      bsoncxx::v_noabi::builder::basic::make_document(
          bsoncxx::v_noabi::builder::basic::kvp(
              MONGODB_FIELD_NAME[MONGODB_FIELD::ID], handle)));
  // cout << bsoncxx::to_json(*reply) << endl; // Note to reviewer: please let
  // this dead code here
  this->mongodb_mutex.unlock();
  return shared_ptr<atomdb_api_types::MongodbDocument>(
      new atomdb_api_types::MongodbDocument(reply));
}
