...
shared_ptr<MessageBroker>
MessageBroker::factory(MessageBrokerType instance_type,
                       shared_ptr<MessageFactory> host_node,
                       const string &node_id) {
  switch (instance_type) {
  case MessageBrokerType::RAM: {
    return shared_ptr<MessageBroker>(
        new SynchronousSharedRAM(host_node, node_id));
  }
...
MessageBroker::MessageBroker(shared_ptr<MessageFactory> host_node,
                             const string &node_id) {
  if (!host_node) {
...
SynchronousSharedRAM::SynchronousSharedRAM(shared_ptr<MessageFactory> host_node,
                                           const string &node_id)
...
SynchronousGRPC::SynchronousGRPC(shared_ptr<MessageFactory> host_node,
                                 const string &node_id)
    : MessageBroker(host_node, node_id) {
  ...
  do {
    ...
    if (request != NULL) {
      ...
      if (message_data->is_broadcast) {
        if (message_data->visited.find(this->node_id) !=
            message_data->visited.end()) {
          continue;
        }
        this->peers_mutex.lock();
        for (auto target : this->peers) {
          if (message_data->visited.find(target) ==
              message_data->visited.end()) {
            CommandLinePackage *command_line = new CommandLinePackage(
                message_data->command, message_data->args);
      ...
      std::shared_ptr<Message> message = this->host_node->message_factory(
          message_data->command, message_data->args);
      if (message) {
  ...
  do {
    ...
    if (request != NULL) {
        ...
        if (num_peers > 0) {
          ...
          for (auto target : this->peers) {
            if (visited.find(target) == visited.end()) {
              ...
              auto stub = dasproto::AtomSpaceNode::NewStub(grpc::CreateChannel(
                  target, grpc::InsecureChannelCredentials()));
              stub->execute_message(&(context[cursor]), *message_data,
                                    &(reply[cursor]));
              cursor++;
            }
          }
      ...
      std::shared_ptr<Message> message =
          this->host_node->message_factory(command, args);
      if (message) {
        message->act(this->host_node);
      } else {
        Utils::error("Invalid NULL Message");
...
void SynchronousSharedRAM::join_network() {
  ...
  for (unsigned int i = 0; i < MESSAGE_THREAD_COUNT; i++) {
    this->inbox_threads.push_back(
        new thread(&SynchronousSharedRAM::inbox_thread_method, this));
  }
...
void SynchronousSharedRAM::broadcast(const string &command,
                                     const vector<string> &args) {
  if (!this->is_shutting_down()) {
    this->peers_mutex.lock();
    unsigned int num_peers = this->peers.size();
...
void SynchronousGRPC::join_network() {
  this->grpc_thread =
      new std::thread(&SynchronousGRPC::grpc_thread_method, this);
  while (!this->grpc_server_started()) {
    Utils::sleep();
  }
  for (unsigned int i = 0; i < MESSAGE_THREAD_COUNT; i++) {
    this->inbox_threads.push_back(
        new thread(&SynchronousGRPC::inbox_thread_method, this));
  }
...
void SynchronousGRPC::send(const string &command, const vector<string> &args,
                           const string &recipient) {
  if (!is_peer(recipient)) {
    Utils::error("Unknown peer: " + recipient);
  }
  ...
  // TODO: Once das-proto is updated, update atom_space_node to
  // distributed_algorithm_node auto stub =
  // dasproto::DistributedAlgorithmNode::NewStub(grpc::CreateChannel(recipient,
  // grpc::InsecureChannelCredentials()));
...
void SynchronousGRPC::broadcast(const string &command,
                                const vector<string> &args) {
  this->peers_mutex.lock();
  unsigned int num_peers = this->peers.size();
  if (num_peers == 0) {
  ...
  for (auto peer_id : this->peers) {
    ...
    // TODO: Once das-proto is updated, update atom_space_node to
    // distributed_algorithm_node auto stub =
    // dasproto::DistributedAlgorithmNode::NewStub(grpc::CreateChannel(peer_id,
    // grpc::InsecureChannelCredentials()));
    auto stub = dasproto::AtomSpaceNode::NewStub(
...
grpc::Status
SynchronousGRPC::execute_message(grpc::ServerContext *grpc_context,
                                 const dasproto::MessageData *request,
                                 dasproto::Empty *reply) {
  if (!this->is_shutting_down()) {
    // TODO: fix memory leak
    this->incoming_messages.enqueue(
        (void *)new dasproto::MessageData(*request));
    return grpc::Status::OK;
  } else {
    return grpc::Status::CANCELLED;
  }
...
CommandLinePackage::CommandLinePackage(const string &command,
                                       const vector<string> &args) {
