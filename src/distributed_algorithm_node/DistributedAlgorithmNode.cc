...
DistributedAlgorithmNode::DistributedAlgorithmNode(
    const string &node_id, LeadershipBrokerType leadership_algorithm,
  ...
  this->message_broker = MessageBroker::factory(
      ...
      shared_ptr<MessageFactory>(this, [](MessageFactory *) {}), node_id);
}
...
void DistributedAlgorithmNode::join_network() {
  ...
  this->message_broker->broadcast(this->known_commands.NODE_JOINED_NETWORK,
                                  args);
}
...
void DistributedAlgorithmNode::broadcast(const string &command,
...
DistributedAlgorithmNode::message_factory(string &command,
                                          vector<string> &args) {
