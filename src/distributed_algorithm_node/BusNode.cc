...
BusNode::BusNode(
    ...
    DistributedAlgorithmNode(
        node_id, 
        LeadershipBrokerType::TRUSTED_BUS_PEER, 
        messaging_backend) {
...
void BusNode::Bus::add(const string &command) {
    if (this->command_owner.find(command) != this->command_owner.end()) {
        if (this->command_owner[command] != "") {
            Utils::error("Bus: command <" + command + "> " + 
                "is already assigned to " + this->command_owner[command]);
        }
    } else {
...
SetCommandOwnership::SetCommandOwnership(
    const string &node_id, 
